...
Replace `<tagName>` with the version you want to download. For example, command below will download the version `1.3.0`
...
2)  Download `renv.lock` :point_right: [here](https://raw.githubusercontent.com/cparsania/FungiExpresZ/master/renv.lock).

`renv.lock` file contains information required to install specific version of dependency packages.

3)  Move `renv.lock` file into the installation directory created in step-1.

4)  Download an appropriate package bundle. :point_right: [Download package bundle here](https://github.com/cparsania/FungiExpresZ/releases)
...
NOTE: To run above commands `renv.lock` file must be in the same directory.
...
> NOTE: In above command `FungiExpresZ_1.3.0.tar.gz` is the path to bundle file downloaded in the step-4.
...
Chirag Parsania,  Ruiwen Chen,  Pooja Sethiya,  Zhengqiang Miao,  Liguo Dong, Koon Ho Wong, FungiExpresZ: an intuitive package for fungal gene expression data analysis, visualization and discovery, *Briefings in Bioinformatics*, Volume 24, Issue 2, March 2023, bbad051, <https://doi.org/10.1093/bib/bbad051>
