...
app_server <- function(input, output,session) {
  # List the first level callModules here
        ...
        strains_by_species <- reactive({
                req(sample_info)
                sample_info %>% dplyr::select(species, strain, genotype) %>%
                        group_by(species) %>% 
                        summarise(genotype = list(genotype) , strain = list(strain)) 
        })
        ...
        sra_sample_info_user_display <- reactive({
                ...
                dd <- sample_info %>% filter(!!!user_filters_expressions())
                return(dd)
        })
        ...
        user_selected_sra_id_data <- eventReactive(input$submit_user_sra_samples,{
                ...
                withProgress(message = "Loadind data from .rds" , {
                        incProgress(0.5)
                        expr_data_mat <- readr::read_rds(paste(expression_mats_rds_dir , expr_data_mat_rds_file , sep = "/"))
                        incProgress(1)
                })
        ...
        user_uploaded_data <- eventReactive(input$submit_user_uploaded_data,{
                ...
                processed_data <- data_from_user %>% 
                        ...
                        filter_if(is.numeric , .vars_predicate = any_vars(. != 0 )) ## select genes which have atleast one non zero value
                
                ## log2 trasform user uploaded data 
                if(input$log_transform_user_upload_data == "log2"){
                        processed_data <- processed_data %>% 
                                filter_if(is.numeric , any_vars( . > 0)) %>% ## remove genes having negative values (value < 0) in any sample
        ...
        observe({
                if(input$upload_sample_data){
                        ...
                        updateProgressBar(session = session , id = "upload_sample_data_pb" , value = 100)
                        
                } else {
                        shinyjs::show(id ="user_data_upload_section")
                        updateProgressBar(session = session , id = "upload_sample_data_pb" , value = 0)
                }
        ...
        observeEvent(input$upload_sample_data,{
                ...
                if(input$upload_sample_data){
                        example_data_file  = "./example_data/all_rna_seq_replicate_mean_log_fpkm_mat2.txt"
        ...
        scatter_plot_elems <- eventReactive(input$generate_scatter, {
                ...
                gff_feat =  ah_data_summary2 %>% 
                        filter(genome == current_scatter_plot_genome()) %>% 
                        dplyr::select(gr_cols) %>% unnest()
        ...
        scatter_plot_brushed_points <- eventReactive(input$plot_brush, {
                ...
                display_data <- scatter_plot_brushed_points %>%
                        ...
                        dplyr::select(1,c("seqnames", "start", "end","width", "strand", "description"), everything())
        ...
        output$brush_table <- DT::renderDataTable({
                ...
                return(scatter_plot_brushed_points() %>%
                               mutate_all(funs(replace_na(as.character(.),"--NA--"))) %>% 
        ...
        multi_scatter_vars_validated <- eventReactive(input$generate_multi_scatter, {
                ...
                col_class <- plot_data() %>% dplyr::select(input$multi_scatter_vars) %>% summarise_all(is.numeric) %>% gather()
        ...
        final_corr_heatbox <- reactive({
                ...
                if(input$corr_heatbox_scale_manual == "auto"){
                        chb <- chb + 
                                scale_fill_gradientn(colours = virdis::viridis(n = input$corr_heatbox_colors) %>% rev())
                }
        ...
        output$corr_heatbox_data <- DT::renderDataTable({
                ...
                return(corr_heatbox_data() %>% 
                               ...
                               as_tibble() %>% 
                               mutate_if(is.numeric, function(i){round(i,2)}))
        },selection  = "none",  
        server = F,
        extensions = "Buttons",
        options = list(
        ...
        boxplt <- eventReactive(input$generate_box, {
                ...
                box_plot_data <- box_plot_user_selected_gene_group_data() %>% 
                        ...
                        gather(key = samples , value = value , -gene_groups) %>% 
                        left_join(group_info$column_groups , by = c(samples = "group_members")) %>% 
                        tidyr::replace_na(list(groups = "No groups assigned")) %>% ## NA will be converted to "No groups assigned"
                        dplyr::mutate(samples = forcats::fct_inorder(samples)) %>%
                        dplyr::mutate(groups = forcats::fct_inorder(groups))# %>%
                ...
                withProgress(message = "Preparing box plot" , {
                        ...
                        if (input$box_plot_pvalue == "TRUE") {
                                ...
                                if(length(box_x_var_type) >= 2){
                                        ...
                                        boxplt <- boxplt + 
                                                stat_compare_means(comparisons = all_combin, 
                ...
                withProgress(message = "Preparing violin plot" , {
                        ...
                        if (input$violin_plot_pvalue == "TRUE") {
                                ...
                                if(length(violin_x_var_type) >= 2){
                                        ...
                                        violinplt <- violinplt + 
                                                stat_compare_means(comparisons = all_combin, method = input$violin_plot_pval_method)  
                                }
                        }
        ...
        barplot <- eventReactive(input$generate_barplot, {
                ...
                barplot_data <- plot_data() %>% 
                        ...
                        gather(key = samples , value = value ,  -GeneNames) %>% 
                        left_join(group_info$column_groups , by = c(samples = "group_members")) %>% 
                        tidyr::replace_na(list(groups = "No groups assigned")) %>% ## NA will be converted to "No groups assigned"
        ...
        line_plot_data <- eventReactive(eventExpr = input$generate_lineplot, {
                ...
                if(input$line_plot_cluster_genes_by == "gene_groups"){
                        ...
                        forPlot <- forPlot %>% 
                                ...
                                mutate(clust = !!as.symbol(column_to_be_joined)) %>% 
                ...
                forPlot <- forPlot %>% 
                        ...
                        add_count() %>% 
                        ungroup() %>%
                        mutate(clust = paste(clust, ":(N=" ,n , ")" , sep = "")) %>% 
                        dplyr::select(-n)  
                ...
                if (input$activate_avg_line_plot == "average") {
                        forPlot <- forPlot %>%
                                group_by(clust) %>%
                                summarise_if(is.numeric, input$avg_line_type)
                }
                
                forPlot <- forPlot %>% 
                        tidyr::gather(key = "variable", value = "value" , input$lineplot_x) %>%
                        mutate(variable =  forcats::fct_inorder(variable))  %>%  ## preserve original order of samples
                        group_by(variable) 
        ...
        line_plt <- eventReactive(eventExpr = input$generate_lineplot, {
                ...
                ### plot
                gp <- line_plot_data() %>% 
                        mutate(clust = forcats::fct_relevel(clust, as.numeric(clust) %>% unique %>% sort %>% as.character)) %>% ## numeric cluster to ordered factor 
        ...
        output$line_plot_clustred_data <- DT::renderDataTable({
                ...
                if(isolate(input$activate_avg_line_plot  != "average")){
                        ...
                        user_selected_species_annot <- ah_data_summary2 %>% 
                                filter(genome == lineplot_reference_annot())%>%
                                dplyr::select(gr_cols) %>% tidyr::unnest()  
                ...
                return(line_plot_data_to_show %>% dplyr::arrange(clust) %>% mutate_if(is.numeric , round  , 3))
        },
        selection  = "none",
        server = F,
        extensions = "Buttons",
        ...
        pca_plot_vars_validated <- eventReactive(input$generate_pca_plot, {
                ...
                withProgress(message = "validate data" , {
                        ...
                        col_class <- plot_data() %>% dplyr::select(input$pca_plot_vars) %>% summarise_all(is.numeric) %>% gather()
        ...
        pca_input_matrix  <- eventReactive(eventExpr = input$generate_pca_plot,{
                ...
                withProgress(message = "Prepare PCA data",{
                        ...
                        plot_data_t <- pca_plot_user_selected_gene_group_data() %>% 
                                ...
                                gather(key = sname, value, -1) %>% 
                                spread(1, value) ## transpose data --> genes will be column and samples will be row 
        ...
        pca_data_sample_kmeans <- reactive({
                ...
                km_data <- tryCatch({
                        ...
                        tt <- tibble(sname = rownames(pca_input_matrix()) , kmeans_clust = km$cluster )  
                }, error = function(x){
                        sendSweetAlert(session = session ,title = "PCA sample K-means error!!!" , 
                                       ...
                                       type = "error")
                        tt <- tibble(sname = rownames(pca_input_matrix()) , kmeans_clust = "All samples")  
                        return(tt)
                })
                return(km_data)
        }) 
        ...
        my_pr_comp_names <- reactive({
                ...
                pc_prop_of_var <- summary(pca_plot_pr_comp())$importance %>%
                        ...
                        gather(pc , value , -feature_type) %>% 
                        filter(feature_type == "Proportion of Variance") %>% 
                        dplyr::select(-feature_type) %>%
                        mutate(value = round(value * 100 ,1)) %>% ## convert to percentage
                        mutate(with_var = paste(pc ," (" , value ,"%",")" , sep = "")) %>% 
                        dplyr::select(-value) 
                
                ## return named vector 
                pc_prop_of_var %>% pull(with_var) %>% rlang::set_names( pc_prop_of_var$pc)
        ...
        pca_data_after_groups_excluded_by_user <- reactive({
                ...
                plot_data <-  pr_comp_derieved_tibble() %>% filter(! xor((groups  %in% user_excluded_groups) ,
                                                                         (kmeans_clust %in% user_excluded_groups)))
        ...
        heatmap_parmas_validated <- eventReactive(input$generate_heatmap, {
                ...
                col_class <- plot_data() %>% dplyr::select(input$heatmap_vars) %>% summarise_all(is.numeric) %>% gather()
        ...
        heatmap_top_variable_genes_df <- eventReactive(eventExpr = input$generate_heatmap, {
                ...
                withProgress(message = "Preparing heatmap data " , {
                        ...
                        row_splt <- row_splt %>% 
                                tibble(x = . ) %>% 
                                dplyr::add_count(x) %>% 
                                mutate(cluster_name = paste(input$hm_cluster_prefix , x ,":(N=", n,")" , sep = "")) %>% 
                                pull(cluster_name) 
                        ...
                        if(input$heatmap_column_clusters_choices == "kmeans"){ #
                                ...
                                column_splt_ss <- column_splt_ss %>% 
                                        mutate(sname =  forcats::fct_relevel(as.character(sname) , group_info$column_groups %>% pull(2) %>% unique()  ))
                        ...
                        column_splt <- column_splt %>% 
                                tibble(x = .) %>% 
                                dplyr::add_count(x) %>% 
                                mutate(cluster_name = paste(input$hm_column_cluster_prefix , x ,"\n(N=", n,")" , sep = "")) %>% 
                                pull(cluster_name) %>% 
        ...
        heatmap <- eventReactive(input$generate_heatmap, {
                ...
                withProgress(message = "Preparing heatmap",{
                        ...
                        hm_colors <- tryCatch({
                                ...
                                sendSweetAlert(session = session , title = "Warning" , 
                                               ...
                                               text = tags$h4("Error while generating user defined colors", 
                                                              br(),
                                                              "Error from" , tags$code("colorRamp2()"),
                                                              br(), 
                                                              tags$code(e),br(),
                        ...
                        if(!is.null(input$heatmap_anno_type)){ ## make sure atleast one annot selected
                                ...
                                ## column box annot 
                                column_box_anno <-  anno_boxplot(heatmap_top_variable_genes_df()$mat ,
                                                                 axis = T ,
                                                                 border = T ,
                                ...
                                if(all(c("show_heatmap_column_groups" , "show_heatmap_column_boxplot") %in% input$heatmap_anno_type)){ ## all annotations
                                        ...
                                        column_ha = HeatmapAnnotation(Groups =  hm_selected_column_groups ,
                                                                      Box = column_box_anno, 
                                        ...
                                        column_ha = HeatmapAnnotation(Groups =  hm_selected_column_groups , col = list(Groups = grp_colors))
                                        
                                        ## only box annot 
                                }else if(input$heatmap_anno_type == "show_heatmap_column_boxplot"){ ## box plot annotations
                                        column_ha = HeatmapAnnotation(box = column_box_anno)
                                }
                        }
                        ...
                        hm <- ComplexHeatmap::Heatmap(heatmap_top_variable_genes_df()$mat, 
                                                      ...
                                                      column_names_gp = gpar(fontsize = input$hm_column_names_font_size),
                                                      ...
                                                      row_names_gp = gpar(fontsize = input$hm_row_names_font_size),
        ...
        active_heatmap_data <- reactive({
                ...
                hm_clust <- data_frame(clust = names(clusts), row_num = clusts) %>% unnest()
                ...
                return(hm_data %>% dplyr::mutate_if(is.numeric, round, 4))
        })
        ...
        output$heatmap_data <- DT::renderDataTable({
                ...
                user_selected_species_annot <- ah_data_summary2 %>% 
                        filter(genome == heatmap_reference_annot()) %>% 
                        dplyr::select(gr_cols) %>% tidyr::unnest()  
        ...
        heatmap_column_cluster <- eventReactive(input$generate_heatmap,{
                ...
                wide <- tibble(cluster = names(hm_column_ord_lst) , column_order = hm_column_ord_lst) %>% 
                        ...
                        summarise(column_labels = list(column_labels)) %>% 
                ...
                ## Long format 
                long <- wide %>% 
                        mutate(column_labels =  colum_chr_to_list(column_labels)) %>% 
                        unnest(column_labels)
        ...
        heatmap_row_cluster <-eventReactive(input$generate_heatmap,{
                ...
                wide  <- active_hm_data %>% 
                        ...
                        group_by(clust) %>% 
                        summarise(row_labels = list(!!as.symbol(cols_of_interest[1] ))) %>% 
                        mutate(count = lengths(row_labels)) %>% 
                        mutate(row_labels = map_chr(row_labels, ~paste0(.x , collapse = ","))) %>% 
                ...
                long <- wide %>% 
                        mutate(row_labels = colum_chr_to_list(row_labels)) %>% 
                        unnest(row_labels)
        ...
        hm_cluster_wise_sample_infor_ll <- eventReactive(input$generate_heatmap, {
                ...
                cluster_wise_sample_infor_ll <- cluster_wise_sample_infor_tbl %>% pull(2) 
                names(cluster_wise_sample_infor_ll) <- cluster_wise_sample_infor_tbl %>% pull(1) 
