...
It contains normalized gene expression values of more than 15,000 NCBI SRA data from \> 15 different fungal species and, gene annotations and GO data of more than 100 different fungal species. For 3 strategies mentioned above, you can generate 12 different data exploratory plots and 6 different GO plots.
...
It allows you to either upload or select data from pre-existing SRA data. Depending upon radio button selected (**Select data** or **Upload/Use example data**) submit button toggle between **Select** and **Upload**. Clicking on either of these buttons result in a pop-up which are explained in Fig-2 ([Select SRA data]) and Fig-3 ([Upload user data]).

2.  [**Assign groups**]{.underline}

Often RNA-seq data contains sample groups (e.g., group samples by replicates, strains, time points etc.) and gene-groups (e.g., differentially expressed genes, genes specific to pathway etc.). Comparison between them could reveal similarities and contrasts between these groups, which leads to unfold meaningful biological insights. **Assign groups** feature allows you to upload user defined **Sample groups** and/or **gene-groups**. Additional info on required file format to assign groups given in the Fig-4 ([Sample groups]) and Fig-5. ([Gene groups]) Once the groups uploaded, you may color, cluster or facet the expression values in different plots according to groups assigned.

3.  [**View active groups**]{.underline}

By clicking on **View active groups** you can check the current active groups (both sample and gene groups).

4.  [**Usage**]{.underline}

It displays the locations across the globe, where FungiExpresZ has been used at least once. Blink sign is the indication of "at this moment" access from particular location. Clicking on anywhere in the map you can get more insights on usage statistics across the globe.

5.  [to]{.underline} 10**.**

Number 5-10 are different tabs - **App**, **About**, **Downloads**, **News & Updates** ,**Citations**, and **Contact**. Although one can anticipate content of each tab from the name further details on each has been given later in the tutorial. Current selected tab in Fig-1 is - **App**. As you can see, it contains 12 different plot panels (default open is **Scatter plot**). Each plot panel is explained later in this tutorial.
...
FungiExpresZ contains \>13,000 pre-processed NCBI SRA data from 8 different fungal species. The values given are normalized gene expressed values (FPKM or RPKM). Some of these data have been obtained from public resources while remaining ones are processed by us. You can select any of these data for the purpose of analysis and visualizations. Once you click on **Select data** button the pop-up will appear as shown in Fig-2.

![FIGURE 2: FungiExpresZ select SRA data.](_step_by_step_guide_files/screen_shots/02_fe_select_data.png){width="592"}

1.  [**Organism**]{.underline}

Drop-down **Organism** allows you to select organism of your choice. Once organism is selected table below will show the data related to selected organism.

2.  [**Strain**]{.underline}and
3.  [**Genotype**]{.underline}

Drop-down **Strain** and **Genotype** allows you to filter table by stain and genotype, respectively. Both these filters works with **AND** operator and therefore, while applying them together will only work if both condition satisfy.

> Currently selection of more than one value are not allowed in each filter.

4.  [**Reset Strain**]{.underline} and
5.  [**Reset Genotype**]{.underline}

These are reset buttons for drop-down **Strain** and drop-down **Genotype,** respectively.

6.  [**Select all rows**]{.underline}

Once the data are filtered (either by organism, strain and/or genotype) you need to select the row(s) from resultant table to make them available for analysis and visualization. As name suggests click on **Select all rows** will select all the rows which are displayed in the table. Alternatively, you may also use `shift` key from the keyboard to select more than one rows simultaneously.

7.  [**Copy**]{.underline}

Click on **Copy** button allows copying data to clipboard. You may paste them to any spreadsheet like program to better organize and understand the data type.

8.  [**Download**]{.underline}

Using button **Download** you can download displayed data in the table to one of these three formats - .csv, .pdf or excel.

9.  [**Column visibility**]{.underline}

Click on **Column visibility** will lists the hidden columns from the table. You may select one or more of them to make them visible in the table being displayed.

10. [**Search**]{.underline}

Besides **Organism**, **Strain** and **Genotype** filters, you can apply free text filter using the text box given under the **Search** title. Input text will be matched against all the columns being displayed in the table and matching rows will be displayed as a result.

11. [**Clear all**]{.underline}

Click on **Clear all** will deselect rows if selected any.

12. [**Submit**]{.underline}

Once the rows selected, hitting **Submit** button will make selected data available for analysis. For selected data, SRA id will be displayed as sample identity in each plot panel.

## Upload user data

Often transcriptomics data found in a tabular like format where columns are samples i.e. replicates, time-points, multiple strain types etc. and rows are genes. Each cell in a table contains normalized gene expression values. You can either upload such a tabular format data in .txt/.csv file format or paste in a text box for the purpose of analysis and visualization. Once you click on the button **Upload** (Fig-1 #2), pop-up showed in the Fig-3 will appear.

![FIGURE 3: FungiExpresZ upload user data.](_step_by_step_guide_files/screen_shots/03_fe_file_upload.png){width="542"}

1.  [**Upload example data**]{.underline}

Upon selection of this check-box example data will be activated.

2.  [**Upload data**]{.underline}
...
3.  [**Select column separator**]{.underline}
...
Often RNA-seq data contain sample groups i.e. replicates, strains, time points etc. and gene-groups i.e. differentially expressed genes, genes specific to pathway etc. Comparison between them could reveal similarities and contrasts between these groups, which may unfold meaningful biological insights. Assign groups feature allows you to upload user defined sample groups and gene groups. Discussed below is the technicalities of the group assignments.
...
Second option to assign the sample groups is via user data upload. You can either upload groups via .txt file or paste data in the given text box. Both these ways require identical data format i.e. a matrix of two columns, where first column is group name and second column is group members. A tab, a semicolon or a comma can be used as column deliminator. First row of the matrix will be considered as column names, which can be any appropriate text. Each group member (column 2) must be assigned to the single group. Group members (column 2), which are used as sample names in the uploaded data will only be used to assign the groups and rest will be discarded.

3.  [**Group by BioProject (NCBI)**]{.underline}

While analyzing the samples from NCBI-SRA data alone or along with user uploaded data, it is important to know which SRA samples are from the same study and which are from different. In background, FungiExpresZ groups NCBI-SRA samples by NCBI-BioProjectID. You can activate SRA sample groups by NCBI-BioProjectID by click on button **Submit** under this panel.

6.  [**Submit**]{.underline}

Click on the submit button will activate the group assignment. It is important to note that every time you change the gene expression data, group data will be lost although the groups are same.

> **NOTE:** To reactivate the same groups you need to click **Submit** again or upload groups in case of different groups.
...
To assign the gene groups, you can either upload groups via .txt file or paste data in the given text box. Both these ways require identical data format i.e. a matrix of two columns, where first column is group name and second column is group members. First row of the matrix will be considered as column names, which you can be any appropriate text. Each group member (column 2) must be assigned to the single group. Group members (column 2), which are used as gene names in the uploaded data will only be used to assign the groups and rest will be discarded.

2.  [**Column separator**]{.underline}

While uploading the groups you can use a tab or a semicolon or a comma as a column deliminator.

**3). Example data snap**

The snapshot showing the format for gene groups.

# Plot panels

FungiExpresZ allows you to generate 12 data exploratory plots - **1)** Scatter plot, **2)** Multi-scatter plot, **3)** CorrHeatBox, **4)** Density plot, **5)** Histogram, **6)** joy-plot, **7)** box-plot, **8)** Violin plot, **9)** Bar plot, **10)** PCA plot, **11)** Line plot and **12)** Heatmap. Each of these has independent panel of necessary inputs, plot output and other plot specific settings. In below section each plot panel and their options are discussed in detail.
...
Multi-scatter plot, CorrHeatBox, Density plot, Histogram, joy-plot, box-plot, Violin plot and PCA plot has input panel, which is shown in Fig-7.
...
2.  [**Select gene group(s)**]{.underline}
...
The purpose of the bar plot is to check the expression of individual gene(s) in multiple samples or sample groups. Bar plot input panel is shown in the Fig-8.
...
Line plot is a powerful way to show the trends of observations / genes across multiple samples. For example, one of the ways to use this plot is to show expression of genes across several time points. FungiExpresZ also allows to cluster observations / genes in both unsupervised and supervised way, and simultaneous visualization of clustered data. Additionally, you can even display average line (mean or median) instead of individual line of each gene / observation within each cluster. The input panel of the line plot has been shown in the Fig-9.
...
4.  [**\# top variable genes to show**]{.underline}

Numeric input is required if the option **'\# top variable genes (By standard deviation)'** is selected. Input number will be used to select the top variable genes ranked by standard deviation as described above.

5.  [**Gene cluster**]{.underline}
...
3.  [**Number of genes to plot**]{.underline}
...
4.  [**\# of top variable genes to show**]{.underline}
...
1.  [**Axis limits**]{.underline}: Click on \`set\` pops-up a text-box which allows users to set minimum and maximum values for X and Y axis.

2.  [**Trend line**]{.underline}**:** Buttons under this option allow turn on or off trend line.
...
3.  [**Color by**]{.underline}**:** Set color for the dots in the scatter plot.
    ...
    -   **gene-groups:** Color dots by gene-groups. Color by this option require users to upload gene-groups. See [Assign groups](#assign-groups). Selecting this option without any groups color dots uniformly.

4.  [**Transparency**]{.underline}**:** A slide-bar to adjust transparency of the dots.

5.  [**Point size**]{.underline}**:** A slide-bar to adjust size of the dots.
...
1.  [**Corr text color**]{.underline}**:** Select a color from the palettes or type-in color hex code (e.g. #FF0000) or name (e.g. red) for the correlation values displayed in the plot.

2.  [**Font size**]{.underline}**: Corr text:** A slide-bar to adjust the size of the correlation values displayed in the plot.
...
1.  [**Number of colors**]{.underline}**:** A slide-bar to select number of colors the plot.

2.  [**Cluster variables**]{.underline}**:** Decides whether to cluster samples or not.
...
3.  [**Method**]{.underline}**:** Display correlation values either by square or circle.

4.  [**Type**]{.underline}**:** Display plot as **Full,** only **Upper half** or **Lower half**.

5.  [**Scale**]{.underline}**:** Adjust scale to map values to colors.
...
6.  [**Show corr value**]{.underline}**:** Decide whether to show correlation values in the plot.

7.  [**Font size**]{.underline}**: Corr text:** A slide-bar to adjust font size of the correlation values.

8.  [**Corr text color**]{.underline}**:** Select a color form the palette for the corr values.
...
1.  [**Fill by:**]{.underline} A variable choice to fill colors in the density curve.
...
2.  [**Separate by**]{.underline}**:** A variable choice to separate (or facet) plots.
...
3.  [**Transparency**]{.underline}**:** A slide-bar to adjust transparency of the plot.
...
3.  [**Bar position**]{.underline}**:** Adjust position of the bars in the histogram.
...
4.  [**Number of bins**]{.underline}**:** A numeric-input to adjust number of bins/bars to show in the histogram.

5.  [**Transparency**]{.underline}**:** A slide-bar to adjust transparency of the plot.
...
1.  [**Y-axis:**]{.underline} A variable choice to show values on Y-axis.
...
2.  [**Fill by**]{.underline}**:** A variable choice to fill colors in the joy-plot.
...
3.  [**Fill Choices**]{.underline}**:** Choices for Pre-defined color palette.

4.  [**Fill reverse**]{.underline}**:** Whether toreverse color scale.

5.  [**Color ridges**]{.underline}**:** Select a color from palette for ridges.

6.  [**Separate by**]{.underline}**:** A variable choice to separate (or facet) plots.
...
1.  [**X-axis**]{.underline}**:** A variable choice to show values on X-axis.
...
2.  [**Fill by**]{.underline}**:** A variable choice to fill colors in the box-plot.
...
3.  [**Separate by**]{.underline}**:** A variable choice to separate (or facet) plots.
...
4.  [**P-value:**]{.underline} Whether to show p-values between observation. User can choice between `t.test` or `wilcox.test` to calculate the pair-wise p-values.

5.  [**Dots**]{.underline}**:** Whether to overlay dots on top of box.

6.  [**Box transparency**]{.underline}: A slide-bar to adjust transparency of the each box.
...
2.  [**Fill by**]{.underline}**:** A variable choice to fill colors in the violin-plot.
...
5.  [**Show quantile**]{.underline}**:** For selected quantile values horizontal lines will be plotted within each violin.

6.  [**Transparency**]{.underline}: A slide-bar to adjust transparency of the each violin.
...
1.  [**X-axis:**]{.underline} A choice of variable to show in X-axis.

    -   **Genes:** genes will be plotted on X-axis.

    -   **Samples:** Samples will be plotted X-axis.

2.  [**Fill by:**]{.underline} A choice of variable to fill colors in bars.
...
3.  [**Select**]{.underline}**:** Choose colors from palette when "Fill by" selected to "Manual".

4.  [**Separate by**]{.underline}**:** A variable choice to separate (or facet) plots.
...
5.  [**Scale**]{.underline}**:** Adjust scale for multiple plots. Useful when plots are separated by one of the above variables.
...
1.  [**Select PCs**]{.underline}**:** Select any two principle components to plot them on the X and Y-axis.

2.  [**Display sample names**]{.underline}**:** Whether to show sample/variable names in the PCA plot.

3.  [**Dot size**]{.underline}**:** A slide-bar to adjust size of the dots displayed corresponding to each sample/variable in the PCA plot.

4.  [**Color by**]{.underline}**:** Choice of the variable to color dots displayed in the PCA plot.
...
5.  [**Select color**]{.underline}**:** Select a color from a palette if a choice for `color by` is `Identical`
...
1.  [**Color**]{.underline}**:** Choice of a variable to color lines in the line-plot.
...
2.  [**Separate by**]{.underline}**:** Choice of a variable to separate line plots.
...
3.  [**Scale**]{.underline}**:** Adjust scale for multiple plots. Useful when plots are separated by gene-groups.
...
4.  [**Line shape**]{.underline}**:** A slide-bar to adjust line shapes.

5.  [**Transparency**]{.underline}**:** A slide-bar to adjust transparency of the each line in the line-plot.

6.  [**Number of columns**]{.underline}**:** A numeric input to adjust the plot arrangement in columns. Useful when line-plots are separated/faceted by gene-groups.

7.  [**Point size**]{.underline}**:** A slide-bar to adjust size of the points in the line-plot.

8.  [**Line size**]{.underline}**:** A slide-bar to adjust size (width) of the each line in the line-plot.
...
1.  [**Row height**]{.underline}**:** A slide-bar to adjust the height of the each row in the heatmap.
2.  [**Column width**]{.underline}**:** A slide-bar to adjust the width of the each column in the heatmap.
...
1.  [**Ontology**]{.underline}**:** Select ontology type - Biological Process, Cellular Component, or Molecular Function - for enrichment analysis.

2.  [**Gene cluster**]{.underline}**:** Select a gene cluster for which GO enrichment analysis to be performed.

3.  [**View type**]{.underline}**:** Select a type of plot to visualize enriched GO terms. Choices are...
...
4.  [**Background Genes to GO mapping**]{.underline}**:** It allows user to select type of background data (Genes to GO mapping) to use for GO enrichment analysis.
...
5.  [**\# of minimum genes in a GO term**]{.underline}**:** User can restrict the GO enrichment output. Only those GO terms will be displayed, which has minimum number of genes more than cutoff value.

6.  [**\# of maximum genes in a GO term**]{.underline}**:** User can restrict the GO enrichment output. Only those GO terms will be displayed, which has maximum number of genes less than cutoff value.

7.  [***p-*****adjust**]{.underline}**:** Choice of methods to calculate adjusted *p-*value.
...
1.  [**X-axis**]{.underline}**:** Choice of a variable (Count or Gene Ratio) to show on X-axis.
2.  [**Color by**]{.underline}**:** Choice of a variable (*P-*value, *P-*adjust or *Q-*value) to color observations.
3.  [**Show terms**]{.underline}**:** Numeric input to show number of GO terms in the plot.\
...
1.  [**Font size: Node label**]{.underline}**:** A slide-bar to adjust font size of the labels to each node.
2.  [**Color by**]{.underline}**:** Choice of a variable to color GO-terms.
3.  [**Layout**]{.underline}**:** Select plot layout.
4.  [**Show terms**]{.underline}**:** A numeric input to show number of GO-terms.
...
1.  **Fon[t size Node label]{.underline}:** A slide-bar to adjust font size of the labels to each node.
2.  [**Layout**]{.underline}**:** Select plot layout.
3.  [**Show terms**]{.underline}**:** A numeric input to show number of GO-terms.
...
1.  [**Number of terms**]{.underline}**:** Number of terms to show in the plot.
2.  [**Line size**]{.underline}**:** A slide-bar to adjust the size of lines.
3.  [**Intersection size title**]{.underline}**:** A slide-bar to adjust font size of the Y-axis title - "**Intersection size".**
4.  [**Intersection size tick labels**]{.underline}**:** A slide-bar to adjust font size of the Y-axis tick labels.
5.  [**Set size**]{.underline}**:** Aslide-bar to adjust font size of the label "**Set size**".
6.  [**Point size**]{.underline}**:** A slide-bar to adjust point size in the plot.
7.  [**Set size tick labels**]{.underline}**:** A slide-bar to adjust the tick labels for the axis - "Set size".
8.  [**Set names**]{.underline}**:** A slide-bar to adjust font size of the set names.
9.  [**Numbers above bars**]{.underline}**:** A slide-bar to adjust font size for the number above bars.
10. [**Main bar color**]{.underline}**:** A choice of the color from a color palette for the main bars.
11. [**Sets bar color**]{.underline}: A choice of the color from a color palette for the set bars.
12. [**Matrix color:** A]{.underline} choice of the color from a color palette for the matrix.
...
1.  [**Select**]{.underline}**:** Choice of a color to presence of a gene in a GO-term.
2.  [**Show terms**]{.underline}**:** A numeric input to show number of terms in the plot.
3.  [**Flip axis**]{.underline}**:** Flip values between X and Y axis.
...
![FIGURE 31: FungiExpresZ Video Tutorial page.](_step_by_step_guide_files/screen_shots/40_fe_video_tutorials.png)
